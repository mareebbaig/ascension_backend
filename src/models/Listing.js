module.exports = function Listing() {
    return {
        ...
        getSingleListing:
            "select Li.title, Li.description ,  li.reason_for_selling , Li.is_auctioned , Li.is_established , Li.seller , Ind.label as industry , C.label AS country , P.asking_price , P.cash_flow , P.gross_revenue , P.inventory_price , P.net_income , Li.assets, Li.opportunities, Li.risks  from listing AS Li INNER JOIN industries AS Ind on Li.industry = Ind.id INNER JOIN locations AS Lo ON Li.location = Lo.id INNER JOIN countries AS C on C.id = Lo.country INNER JOIN prices AS P ON Li.price = p.id where Li.id = ${businessId}",
    };
};
